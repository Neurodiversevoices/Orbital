...
import { CapacityLog, ExportConfig, ExportRange, ExportFormat, SupportedLocale } from '../../types';
...
export async function exportNinetyDaySummary(locale: SupportedLocale = 'en') {
  return exportData({ format: 'summary', range: '90d', includeNotes: false }, locale);
}

export async function exportAnnualSummary(locale: SupportedLocale = 'en') {
