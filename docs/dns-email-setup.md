...
**Date:** February 2026
**Domains:** `orbitalhealth.app` (primary), `go.orbitalhealth.app` (Apollo cold outreach subdomain)
**Current email host:** Zoho Mail (for `orbitalhealth.app`)
...
1. **`go.orbitalhealth.app`** — New Zoho subdomain for Apollo cold outreach (`eric@go.orbitalhealth.app`)
2. **`orbitalhealth.app`** — Resend verification for transactional email (waitlist welcome emails from `eric@orbitalhealth.app`)

**Important:** A domain can only have ONE SPF record. Since `orbitalhealth.app` already uses Zoho, the root SPF record must include BOTH Zoho and Resend via `include:` directives.

---

## Part 1: `go.orbitalhealth.app` — Zoho Subdomain for Apollo

### Step 1: Add subdomain in Zoho Mail Admin Console

1. Go to **Zoho Mail Admin Console → Domains → Add Domain**
2. Add `go.orbitalhealth.app` as a subdomain
3. Create the mailbox `eric@go.orbitalhealth.app`

### Step 2: DNS Records to Add

All records below go in your domain registrar's DNS dashboard.

#### MX Records (receive mail at `go.orbitalhealth.app`)
...
> Go to **Zoho Admin Console → go.orbitalhealth.app → Email Configuration → DKIM → Add**. Select 2048-bit key length. Copy the full TXT value provided and paste it as the record value.

#### DMARC Record

| Type | Host/Name   | Value                                                                 | TTL  |
|------|-------------|-----------------------------------------------------------------------|------|
| TXT  | `_dmarc.go` | `v=DMARC1; p=none; rua=mailto:dmarc@go.orbitalhealth.app; pct=100; adkim=r; aspf=r` | 3600 |

> Start with `p=none` (monitoring only). After confirming deliverability, escalate to `p=quarantine` then `p=reject`.

---

## Part 2: `orbitalhealth.app` — Resend for Transactional Email

### Step 1: Set up in Resend Dashboard

1. Go to **[resend.com/domains](https://resend.com/domains)** → Add Domain → `orbitalhealth.app`
...
Resend uses a `send` subdomain (e.g., `send.orbitalhealth.app`) for SPF/bounce processing, and puts DKIM records on your root domain.
...
**Critical:** `orbitalhealth.app` likely already has an SPF record for Zoho:
...
If `orbitalhealth.app` doesn't already have a DMARC record, add one:

| Type | Host/Name | Value                                                                       | TTL  |
|------|-----------|-----------------------------------------------------------------------------|------|
| TXT  | `_dmarc`  | `v=DMARC1; p=none; rua=mailto:dmarc@orbitalhealth.app; pct=100; adkim=r; aspf=r` | 3600 |

> If a DMARC record already exists, **do not add a second one.** Only one DMARC record is allowed per domain. Using `aspf=r` (relaxed) is important because Resend sends from a `send.orbitalhealth.app` subdomain — relaxed alignment allows this to pass.
...
| `RESEND_FROM_EMAIL` | `eric@orbitalhealth.app`                  |

---

## Complete DNS Record Summary

### Records for `orbitalhealth.app` (root domain)

| #  | Type  | Host/Name           | Value                                                     | Priority | Notes                        |
|----|-------|---------------------|-----------------------------------------------------------|----------|------------------------------|
| 1  | TXT   | `@`                 | `v=spf1 include:zohomail.com include:amazonses.com ~all`  | —        | **UPDATE** existing SPF      |
| 2  | TXT   | `_dmarc`            | `v=DMARC1; p=none; rua=mailto:dmarc@orbitalhealth.app; pct=100; adkim=r; aspf=r` | — | Add if doesn't exist |
...
### Records for `send.orbitalhealth.app` (Resend bounce subdomain)
...
### Records for `go.orbitalhealth.app` (Zoho / Apollo subdomain)
...
| 13 | TXT  | `_dmarc.go`           | `v=DMARC1; p=none; rua=mailto:dmarc@go.orbitalhealth.app; pct=100; adkim=r; aspf=r` | — | DMARC for subdomain |
...
- [ ] **Zoho:** Admin Console → go.orbitalhealth.app → Email Configuration → Verify All Records
- [ ] **Zoho:** Send a test email from `eric@go.orbitalhealth.app` and check headers for SPF/DKIM pass
- [ ] **Resend:** Dashboard → Domains → orbitalhealth.app → Verify DNS Records (should show "Verified")
- [ ] **Vercel:** Add `RESEND_API_KEY` and `RESEND_FROM_EMAIL` env vars, redeploy
- [ ] **Apollo:** Settings → Mailboxes → Add `eric@go.orbitalhealth.app` → Run diagnostics
- [ ] **Test:** Submit a waitlist signup on the landing page and confirm welcome email arrives
- [ ] **MXToolbox:** Check `go.orbitalhealth.app` at mxtoolbox.com for SPF/DKIM/DMARC pass
- [ ] **MXToolbox:** Check `orbitalhealth.app` SPF record shows both `zohomail.com` and `amazonses.com`
...
|------|-------------|--------------------------------------------------|
| 1-2  | `p=none`    | Monitor reports at `dmarc@orbitalhealth.app`        |
